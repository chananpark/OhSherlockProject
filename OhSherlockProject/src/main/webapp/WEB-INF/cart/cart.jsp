...
<script>
	...
	function goAllOrder() {
       ...
       for(var i=0; i<totalPriceArr.length; i++) {
			sumtotalPrice += Number(totalPriceArr[i]); 
       }
...
<div class="container" id="cartContainer">
			...
			<tbody>
	            ...
	            <c:if test="${not empty requestScope.cartList}">
               		<c:forEach var="cartvo" items="${requestScope.cartList}" varStatus="status"> 
						...
						<input type="hidden" name="pnumjoin" value="" />
						<input type="hidden" name="oqtyjoin" value="" />
						<input type="hidden" name="cartnojoin" value="" />
						<input type="hidden" name="totalPricejoin" value="" />
	...
	<div class="cartButtons mt-5">
		<span class="float-right"><input class="paymentBtn btn" type="button" value="전체상품 주문" onclick="goAllOrder()"/></span>
		<span class="float-right mr-3"><input type="button" class="btn bg-light text-dark" value="선택상품 주문" onclick="goChkOrder()"/></span>
