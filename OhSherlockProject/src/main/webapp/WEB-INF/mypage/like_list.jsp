					...
					<c:if test="${not empty requestScope.likeList}">
					  <c:forEach var="likevo" items="${requestScope.likeList}" varStatus="status">  <%-- likList 의 구성은 likevo 로 되어있음  --%>
						 <tr>
							...
							<td>
								<%-- 장바구니에 추가해주기★ --%>
								<p><input type="button" class="paymentBtn" onclick="goCart();" value="장바구니"/></p>  
								<%-- 찜목록에서 해당 특정 제품 비우기 --%> 
								<p><input type="button" onclick="goDel('${likevo.likeno}')" value="상품삭제"/></p> <%-- 찜목록번호(시퀀스)로 구별하여 삭제해준다. ${likevo.likeno} 은 숫자이든 문자이든 문자열타입으로 적용될 수 있도록 '' 를 감싸준다. --%>
							</td> 
						 </tr>
				      </c:forEach>
					</c:if>
