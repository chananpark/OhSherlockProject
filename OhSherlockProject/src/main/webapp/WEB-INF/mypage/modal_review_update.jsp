...
<form name="reviewFrm"
      action="<%= request.getContextPath()%>/shop/reviewUpdate.tea"   <%-- action 은 본인페이지로 보낸다. --%>
      method="post"
      enctype="multipart/form-data">
 
<c:if test="${not empty requestScope.rvo}">
   <c:forEach var="reivewvo" items="${requestScope.rvo}" varStatus="status">  <%-- reviewList 의 구성은 reivewvo 로 되어있음  --%>
	<ul style="list-style-type: none">
    	<li style="margin: 25px 0">
    		<input type="text" name="userid" value="${sessionScope.loginuser.userid}" />  <%-- 사용후기를 적은 사용자 userid --%>
    		<input type="text" name="pnum" value="${requestScope.rvo.pnum}" />      <%-- 사용후기를 적은 상품번호 --%>
	...
	<div class="my-3">
	    <p class="text-center">
	       <button type="button" class="btn" id="btnCommentOK" style="background-color: #1E7F15; color: white;">상품 후기 수정하기</button> <%-- 수정하기 버튼을 클릭시 ajax 로 들어간다. --%>
	    </p>
  	</div>
