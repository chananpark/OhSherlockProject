...
<script type="text/javascript">
	...
	function registerBtn(pnum,userid) {
		/*
	...
	*/
	...
	//else {  // 상품을 구매한 경우
		...
		const review_score = $("input[name='review_score']:checked").val();          // 평점
		...
		*/
		...
		$.ajax({
		    ...
		    type:"POST",
		    data:{"review_subject":review_subject,
		    ...
		    success:function(json) {  // {"n":1} 또는 {"n":0}
		    	...
		    	$("input[name='subject']").val("");  // 상품리뷰 제목을 비우고, 마우스 커서 놓아준다. 다음 상품리뷰 작성시를 위해서 비워준다.
...
<form name="reviewFrm">
	...
	<div class="my-3">
	    <p class="text-center">
	       <button type="button" class="btn" id="btnCommentOK" style="background-color: #1E7F15; color: white;" onclick="registerBtn('${requestScope.pvo.pnum}', '${sessionScope.loginuser.userid}');">상품 후기 등록하기</button> <%-- 후기등록 버튼을 클릭시 ajax 로 들어간다. --%>
	    </p>
  	</div>
